...
public static class Extensions
{
    ...
    public static bool IsNull<T>(T? nullable) where T : struct => nullable == null;
    ...
    public static string Prepend(this string str, string prefix) => prefix + str;

    public static IEnumerable<T> PrependAll<T>(this IEnumerable<T> sequence, params T[] newItems) => 
        newItems.Concat(sequence);
    ...
    public static IEnumerable<PropertyInfo> GetPropertiesWithAttribute(this Type type, Type attrType, Func<Attribute, bool> checker = null)
    {
        return type.GetProperties()
            .Where(x => Array.Exists(x.GetCustomAttributes(true),
                    attr => attr.GetType() == attrType && (checker?.Invoke(attr as Attribute) ?? true)
                )
            );
    }
