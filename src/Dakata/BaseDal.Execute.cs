...
namespace Dakata
{
    public partial class BaseDal
    {
        ...
        protected virtual T Execute<T>(Func<IDbConnection, T> func) => DapperConnection.Execute(func);
        protected virtual async Task<T> ExecuteAsync<T>(Func<IDbConnection, Task<T>> func) => await DapperConnection.ExecuteAsync(func);
        ...
        protected virtual IEnumerable<dynamic> QueryDynamic(string sql, object parameters) => DapperConnection.Query<dynamic>(sql, parameters);
        protected virtual async Task<IEnumerable<dynamic>> QueryDynamicAsync(string sql, object parameters) => await DapperConnection.QueryAsync<dynamic>(sql, parameters);
    }
